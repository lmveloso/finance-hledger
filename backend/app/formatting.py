...
def display_segment(segment: str) -> str:
    """Lookup display name for a segment, capitalizing unknown segments."""
    return _DISPLAY_NAMES.get(segment, segment.capitalize())
...
def format_category(account: str) -> str:
    ...
    parts = account.split(":")
    if len(parts) >= 2:
        return _DISPLAY_NAMES.get(parts[-1], parts[-1].capitalize())
    return account
...
def format_account_name(account: str) -> str:
    ...
    if len(parts) >= 3:
        ...
        leaf = _DISPLAY_NAMES.get(parts[-1], parts[-1].capitalize())
        parent = _DISPLAY_NAMES.get(parts[-2], parts[-2].capitalize())
...
def category_spending(month: str) -> dict[str, float]:
    ...
    begin, end = (
        main.month_bounds(month)
        if hasattr(main, "month_bounds")
        else (f"{month}-01", f"{month}-28")
    )
    ...
    data = main.hledger(
        "balance", "expenses", "--depth=2", "-b", begin, "-e", end, "--layout=bare"
    )
    cats = {}
